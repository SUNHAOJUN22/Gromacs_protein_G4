...
# 读取 hbdist.xvg 文件
file_name = 'hbdist.xvg'  # 修改为你的文件路径
...
plt.plot(distances, density, color='skyblue', label='Hydrogen Bond Distance Distribution')

# 设置标题和标签
plt.title('Hydrogen Bond Distance Distribution', fontsize=14)
...
# 保存图像到文件（例如保存为 'hbdist_distribution.png'）
output_file = 'hbdist_distribution.png'  # 可以更改为其他路径或文件名
